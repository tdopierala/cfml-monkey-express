<cfoutput>
	...
	<div class="wrapper">
		...
		<h3>Archiwalna lista indeksów</h3>
		<!---<br /><br />
		<h4>Funkcjonalność chwilowo niedostepna</h4>--->
		...
		<cfif _index is true>
			<ol class="productlinks">
				<!---<li>#linkTo(text="Dodaj nowy indeks", action="new")#</li>--->
				<li>#linkTo(text="Powrót do nowych indeksów", controller="indexes", action="index")#</li>
			</ol>
		</cfif>
...
<script>
	...
	if (typeof(localStorage.productsort) != 'undefined') {
		
		param = JSON.parse(localStorage.getItem('productsort'));
	...
	function refresh(_pn, _pt, _ps, _pu, _pc, _page, _dfrom, _dto, view){
		...
		delete localStorage['productsort'];
					
		localStorage.setItem('productsort', JSON.stringify(param_tab));
		
		$.ajax({
			type		:	'get',
			dataType	:	'html',
			url		:	<cfoutput>"#URLFor(controller='products',action='index')#"</cfoutput> + '&pn=' + _pn + '&pt=' + _pt + '&ps=' + _ps + '&pu=' + _pu + '&pc=' + _pc + '&page=' + (_page-1) + '&dfrom=' + _dfrom + '&dto=' + _dto + '&view=' + view,
	...
	$(function(){
		...
		$(".product_xls").on("click", function(){
			...
			document.location = <cfoutput>"#URLFor(controller='products',action='index')#"</cfoutput> + '&pn=' + $("#pn").val() + '&pt=' + $("#pt").val() + '&ps=' + $("#ps").val() + '&pu=' + $("#pu").val() + '&pc=' + $("#category").val() + '&page=' + 0 + '&dfrom=' + from + '&dto=' + to + '&view=' + 'xls';
