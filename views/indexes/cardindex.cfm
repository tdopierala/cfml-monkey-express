...
<script>
	...
	if (typeof(localStorage.productsortcard) != 'undefined') {
		
		param = JSON.parse(localStorage.getItem('productsortcard'));
	...
	function refresh(_pn, _pt, _ps, _pu, _pc, _page, _dfrom, _dto, view){
		...
		_pt = typeof _pt !== 'undefined' ? _pt : 0;
		_pc = typeof _pc !== 'undefined' ? _pc : 0;
		_pu = typeof _pu !== 'undefined' ? _pu : 0;
		_pu = typeof _pu !== 'undefined' ? _pu : 0;
		_ps = typeof _ps !== 'undefined' ? _ps : 0;
		...
		delete localStorage['productsortcard'];
		
		localStorage.setItem('productsortcard', JSON.stringify(param_tab));
		
		$.ajax({
			type		:	'get',
			dataType	:	'html',
			url		:	<cfoutput>"#URLFor(action='card-index')#"</cfoutput> + '&pn=' + _pn + '&pt=' + _pt + '&ps=' + _ps + '&pu=' + _pu + '&pc=' + _pc + '&page=' + (_page-1) + '&dfrom=' + _dfrom + '&dto=' + _dto + '&view=' + view,
