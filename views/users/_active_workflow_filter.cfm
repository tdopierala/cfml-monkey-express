<cfoutput>
	...
	<cfform
		name="active_workflow_filter_form"
		action="#URLFor(controller='Users',action=params.action,key=session.user.id)#"
		onsubmit="javascript:ColdFusion.navigate('#URLFor(controller='Users',action=params.action,key=session.user.id)#', 'intranet_left_content', null, null, 'POST', 'active_workflow_filter_form');return false;" >
			...
			<tbody>
				<tr>
					...
					<td class="label_column" style="text-align:center;">
						<a
							href="javascript:ColdFusion.navigate('#URLFor(controller="Users",action="#params.action#",key=session.user.id,params="all=1")#', 'intranet_left_content');"
							class="all_link"
